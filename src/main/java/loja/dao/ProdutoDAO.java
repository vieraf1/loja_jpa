...
public class ProdutoDAO {
	...
	public List<Produto> buscarPorParametrosComCriteria(String nome, BigDecimal preco, LocalDate dataCadastro) {
		...
		if(nome != null && !nome.trim().isEmpty()) {
			predicate = builder.and(predicate, builder.equal(from.get("nome"), nome));
		}
		
		if(preco != null) {
			predicate = builder.and(predicate, builder.equal(from.get("preco"), preco));
		}
		
		if(dataCadastro != null) {
			predicate = builder.and(predicate, builder.equal(from.get("dataCadastro"), dataCadastro));
		}
